...
import { CreditUnderwriter } from './components/CreditUnderwriter';
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: '900px', margin: '0 auto' }}>
      <header style={{ padding: '1.5rem', background: '#0f172a', color: '#fff', borderRadius: '8px', marginBottom: '1.5rem' }}>
        <h2 style={{ margin: 0 }}>SME Finance Operations & Credit Underwriting AI</h2>
      ...
      <main style={{ display: 'grid', gap: '1.5rem' }}>
        <CashFlowForecaster onTriage={setData} onScreen={setData} onDiagnose={setData} onCalculate={setData} onExecute={setData} onScan={setData} />
        <CreditUnderwriter triageResult={data} candidate={data} result={data} data={data} trajectory={data} audit={data} />
      </main>
    </div>
  );
}
